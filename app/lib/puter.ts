...
export const usePuterStore = create<PuterStore>((set, get) => {
  ...
  const chat = async (
    ...
    try {
      return (await puter.ai.chat(prompt, imageURL, testMode, options)) as AIResponse;
    } catch (err: any) {
      const msg =
        err?.error ||
  ...
  // ✅ FIXED HERE: removed invalid model and added try/catch
  ...
  const img2txt = async (image: string | File | Blob, testMode?: boolean) => {
    ...
    try {
      ...
      const msg =
        err?.error ||
        (err instanceof Error ? err.message : "img2txt failed");
      setError(String(msg));
